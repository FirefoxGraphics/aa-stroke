...
impl RecursiveFlattener {
    pub fn new(bezier: &CBezier,
        sink: Box<dyn CFlatteningSink>,
            // The reciptient of the flattened data
        tolerance: f64)       // Flattening tolerance
        -> Self 
    {
        RecursiveFlattener { bezier: bezier.clone(), sink, tolerance }
    }
    pub fn Flatten(&mut self) {
        let v212 = self.bezier.m_ptB[1] - self.bezier.m_ptB[0];
        let v214 = self.bezier.m_ptB[2] - self.bezier.m_ptB[1];
        let v216 = self.bezier.m_ptB[3] - self.bezier.m_ptB[2];
        let v218 = v214 - v212;
        let v220 = v216 - v214;
